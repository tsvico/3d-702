...
> PS: 目前只支持 `6面图`的全景照片，即前(f)、后(b)、左(l)、右(r)、上(u)、下(d)  
> 如果有现成的`全景图`，可以使用[Panorama to Cubemap
> ](https://jaxry.github.io/panorama-to-cubemap/)将其转为`6面图`。

> 如果有切片成更细格式的图（有些网站会对一个面图片切成多张小图，以加载更高清照片，以 f_1_1.jpg、f_1_2.jpg、f_2_1.jpg、f_2_2.jpg 等命名），请使用工具合并成全景图，再转换成`6面图`
...
如果 F12 觉得太慢，可以使用 [VR_justeasy_krpano_downloader](https://github.com/egdw/VR_justeasy_krpano_downloader/tree/master) 工具下载。
